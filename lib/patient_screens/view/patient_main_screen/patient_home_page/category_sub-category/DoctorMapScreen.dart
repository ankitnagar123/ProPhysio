...
class _MapViewScreenState extends State<MapViewScreen> {
    ...
    setState(() {
      ...
      for (int i = 0; i < doctorListCtr.doctorList.length; i++) {
        ...
        final LatLng latLng = LatLng(
          double.parse(doctorListCtr.doctorList[i].branchLat.toString()),
          double.parse(doctorListCtr.doctorList[i].branchLong.toString()),
        );
        markers[MarkerId('marker_id_$i')] = Marker(
          icon: markerIcon,
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance!.addPostFrameCallback((timeStamp) {
      doctorListCtr.doctorlistfetch(context, widget.catId, "", "", "", "", "");
      log("MAP page");
    });
  }
        ...
        return Stack(
          children: [
            GoogleMap(
              ...
              initialCameraPosition: CameraPosition(
                target: LatLng(
                  doctorListCtr.doctorList.isEmpty
                      ? double.parse("21.7679")
                      : double.parse(doctorListCtr.doctorList[0].branchLat.toString()),
                  doctorListCtr.doctorList.isEmpty
                      ? double.parse("78.8718")
  ...
  Widget buildInfoWindowContent(int i) {
    return GestureDetector(
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => DoctorDetailScreen(
              ...
              branchId: doctorListCtr.doctorList[i].branchId,
              drImg: doctorListCtr.doctorList[i].doctorProfile.toString(),
              cat: widget.catId,
            ),
          ),
        );
      ...
      child: Card(
        margin: const EdgeInsets.symmetric(horizontal: 7, vertical: 6.0),
        color: MyColor.white,
        elevation: 2.2,
        child: Row(
          children: [
            SizedBox(
              ...
              child: FadeInImage.assetNetwork(
                placeholder: "assets/images/loading.gif",
                alignment: Alignment.center,
                image: doctorListCtr.doctorList[i].doctorProfile.toString(),
                fit: BoxFit.fitWidth,
                width: double.infinity,
                imageErrorBuilder: (context, error, stackTrace) {
                  return Image.asset(
            ...
            Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    const Icon(Icons.location_on_outlined, size: 18, color: MyColor.primary1),
                    SizedBox(
                      width: 150,
                      child: Text(
                        doctorListCtr.doctorList[i].branchAddress.toString(),
                        maxLines: 2,
                        softWrap: true,
                        overflow: TextOverflow.ellipsis,
                      ),
                ...
                SizedBox(
                  child: customView.text(
                    doctorListCtr.doctorList[i].category.toString(),
                    12,
                    FontWeight.w500,
                    MyColor.black,
                  ),
